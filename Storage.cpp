...
//    }
	...
	//	The Caching version of the readBlock -----------------------------------
    StatusResult Storage::readBlock(int aBlockNumber, Block &aBlock, std::size_t aBlockSize) {
        if(!cache.hasBlock(aBlockNumber)) {
            stream.seekg(aBlockNumber * sizeof(Block));
            if(!stream.read ((char*)&aBlock, aBlockSize)) {
                return StatusResult(readError);
            }
            cache.cacheBlock(aBlockNumber, aBlock);
        }
        else
            aBlock=cache.getBlock(aBlockNumber);
        return StatusResult{noError};
    }
...
//    }
	...
	//	The Caching version of the writeBlock ----------------------------------
    StatusResult Storage::writeBlock(int aBlockNum, Block &aBlock) {
        stream.seekg(stream.tellg(), std::ios::beg); //attemp to sync stream buffers...
	...
	bool BlockCache::hasBlock(int theBlockNum) {
//        if (!queue.empty()) {
//        std::list<Brick>::iterator end = queue.end();
//            for (std::list<Brick>::iterator it = queue.begin() ; it != end; ++it) {
//                if (it->blocknumber == theBlockNum) {
//                    it->count++;
//                    return true;
//                }
//            }
//        }
//        return false;
        if(!hash.count(theBlockNum))
            return false;
        else
            return true;
	}

	Block& BlockCache::getBlock(int theBlockNum) {
//        std::list<Brick>::iterator end = queue.end();
//        for (std::list<Brick>::iterator it = queue.begin() ; it != end; ++it) {
//            if (it->blocknumber == theBlockNum) {
//                it->count++;
...
//        return hash2[theBlockNum];
	...
	void BlockCache::cacheBlock(int theBlockNum, Block& theBlock) {
		// aNewBlock.blocknumber = theBlockNum;
//        Brick aPair(theBlockNum,0,theBlock);
//        uint8_t min(0);
//        size_t current = queue.size();
//        if (current > size) {
//            std::list<Brick>::iterator drop;
//            std::list<Brick>::iterator it = queue.begin();
//            std::list<Brick>::iterator end = queue.end();
//              for (; it!= end; ++it) {
//                if(it->count <= min) {
//                    min = it->count;
...
//        }
//        queue.push_front(aPair);
//        return;
        
//        hash2[theBlockNum] = theBlock;
        theBlock.header.blockNumber=theBlockNum;
        lru.push_front(theBlock);
        hash[theBlockNum] = std::make_pair (theBlock, lru.begin());
        if (size < lru.size()) {
//            std::list<Block>::iterator it = lru.end();
//            it--;
            hash.erase(lru.end()-- ->header.blockNumber);
            lru.pop_back();
        }
	}
    
    void BlockCache::updateLocation(int aBlockNum, Block aBlock){
        aBlock.header.blockNumber = aBlockNum;
        lru.push_front(aBlock);
        std::list<Block>::iterator it = hash[aBlockNum].second;
        lru.erase(it);
        hash[aBlockNum].second = lru.begin();
    }
