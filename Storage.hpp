...
namespace ECE141 {
	...
	struct Block {
		...
		Block(const Block &aBlock) {
			if (aBlock.header.type == kDataBlockType) {
				for (int i = 0; i < kPayloadSize; i++) {
					data[i] = aBlock.data[i];
				}
			}
			header = aBlock.header;
		}
	...
	public:
		...
		bool hasBlock(int aBlockNum);
		//	This should increment values that are there, and if it isn't there add.
		Block& getBlock(int aBlockNum);
		//	This should sort as things get added.
		void cacheBlock(int aBlockNum, Block& aBlock);
