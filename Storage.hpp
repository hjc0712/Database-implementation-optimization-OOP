...
namespace ECE141 {
  ...
  struct Block {
    ...
    Block(unsigned char aType='D', uint32_t id=0) : header(aType, id) {
      memset(data,0,sizeof(data));
    }
    
    Block(BlockHeader &aHeader) : header(aHeader), data() {}
    Block(const KeyValues &aKVList);  //encode data into the block...
    Block(const Row& aRow);
    void construct (const KeyValues &aKVList);
    BlockHeader   header;
    union {
      unsigned char            data[kBlockSize-sizeof(BlockHeader)];
      //SchemaDef     later...
